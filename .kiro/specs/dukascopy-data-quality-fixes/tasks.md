...
- [ ] 1. Fix `IntervalToMinutes` in `DukascopyHelpers`
  ...
  - [ ] 1.1 Remove the `_ => 1440` fallback arm and throw on unrecognized input
...
- [ ] 2. Fix OHLC integrity in `Aggregate()` and `ParseCandles()`
  ...
  - [ ] 2.3 Add `h >= l` guard to `ParseCandles()` source bar filter

    - Change `if (o > 0 && h > 0 && l > 0 && c > 0)` to `if (o > 0 && h > 0 && l > 0 && c > 0 && h >= l)`
    - _Requirements: 2.4_

- [ ] 3. Implement per-day cache strategy in `DukascopyDataProvider`

  - [ ] 3.1 Add `GetDayCachePath` helper to `DukascopyHelpers`
  ...
  - [ ] 3.2 Refactor `DukascopyDataProvider.GetBars()` to use per-day cache
...
- [ ] 4. Implement tick data in `DukascopyDataProvider.GetTicks()`

  - [ ] 4.1 Add `ParseTicks` method to `DukascopyHelpers`
  ...
  - [ ] 4.2 Implement `GetTicks()` in `DukascopyDataProvider`
...
- [ ] 5. Add ASK candles and mid-price option

  - [ ] 5.1 Add `DukascopyPriceType` enum and constructor parameter to `DukascopyDataProvider`

    - Enum values: `Bid` (default), `Ask`, `Mid`
    - Default behaviour unchanged when `PriceType` is not specified
    - _Requirements: 5.1, 5.4_

  - [ ] 5.2 Update `BuildDayUrl` to accept price type and select the correct .bi5 file

    - `Bid` → `BID_candles_min_1.bi5`, `Ask` → `ASK_candles_min_1.bi5`
    - _Requirements: 5.2_

  - [ ] 5.3 Implement mid-price computation in `GetBars()` when `PriceType == Mid`
...
- [ ] 6. Fix portable endianness in `Decompress()`

  - [ ] 6.1 Replace `BitConverter.ToInt64(data, 5)` with `BinaryPrimitives.ReadInt64LittleEndian(data.AsSpan(5, 8))`

    - Import `System.Buffers.Binary` namespace if not already present
    - No other changes to decompression logic
    - _Requirements: 6.1, 6.2, 6.3, 6.4_

- [ ] 7. Add partial session bar warning logging

  - [ ] 7.1 Detect and log partial opening bars in `GetBars()`
...
- [ ] 8. Add HTTP retry with exponential back-off

  - [ ] 8.1 Add Polly retry policy to `DukascopyDataProvider` HTTP requests
...
- [ ] 9. Write tests in `DukascopyHelpersTests` (in `IntegrationTests/MarketData/DukascopyHelpersTests.cs`, extending the existing class)

  - [ ] 9.1 Add interval parsing tests — valid inputs

    - `[Theory]` covering: `1m`, `5m`, `15m`, `30m`, `1h`, `1H`, `60m`, `4h`, `4H`, `1d`, `1D`, `daily`, `Daily`
    - Each asserts the correct minute count is returned
    - _Requirements: 9.2_

  - [ ] 9.2 Add interval parsing tests — invalid inputs
  ...
  - [ ] 9.3 Add OHLC aggregation tests
  ...
  - [ ] 9.7 Verify all tests pass and no existing tests regress
