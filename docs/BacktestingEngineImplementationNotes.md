...
`IStrategy` defines three methods, two of which have default no-op implementations:

| Method | When Called | Purpose |
|---|---|---|
| `Initialize(StrategyConfig config)` | Once before the first bar of a new execution window | Sets parameters from config; alternative to constructor injection |
| `Reset()` | Between walk-forward windows (or any reuse scenario) | Clears all indicator state and internal tracking so the instance behaves like a fresh one |
| `OnMarketData(MarketDataEvent evt)` | Every bar/tick during the heartbeat loop | Core signal generation; returns `IReadOnlyList<EngineEvent>` |

`Initialize` and `Reset` are default interface methods (no-op if not overridden). Strategies that only use constructor injection for parameters can ignore both. Walk-forward workflows call `Reset()` between out-of-sample windows to reuse the same instance without reconstruction overhead. `IStrategyFactory.Create(config)` is the preferred mechanism for parallel workflows that need fully isolated instances.
...
| SharpeRatio | equity curve | Annualised, risk-free rate from config |
| SortinoRatio | equity curve | Downside deviation over ALL returns (upside zeroed); annualised via `BarsPerYear` |
...
| HistoricalVaR | equity curve + confidence | Value at Risk at a given confidence level (e.g. 0.95); returns the loss at the percentile cutoff as a positive number; null when fewer than 30 period returns |
| HistoricalCVaR | equity curve + confidence | Conditional VaR (Expected Shortfall) — average of returns in the tail beyond the VaR cutoff; null when fewer than 30 period returns |
...
## Trade Excursion Tracking (MAE/MFE)

`ClosedTrade` carries two levels of MAE/MFE data:

### Always-Tracked Fields (on ClosedTrade directly)

- `MaxAdverseExcursion` (decimal, default 0) — the worst (most negative) unrealised P&L observed between entry and exit, in absolute currency terms.
- `MaxFavorableExcursion` (decimal, default 0) — the best (most positive) unrealised P&L observed between entry and exit, in absolute currency terms.

These fields are always populated regardless of trace mode. They enable edge ratio, R-multiple distribution, and entry/exit quality scoring downstream without requiring the full event trace overhead.

### Trace-Only Anatomy (TradeAnatomy record)

When `TraceOptions.EnableEventTrace` is active, the engine additionally attaches a `TradeAnatomy` record to each `ClosedTrade` on exit with richer intra-trade analytics.
...
MAE/MFE data (both the always-tracked currency fields and the trace-only fractional anatomy) enables edge ratio analysis, R-multiple distributions, entry/exit quality scoring, and duration distribution analytics in research workflows.
