...
- Existing endpoints (`POST /scenarios/run`, `/sweep`, `/montecarlo`, `/walkforward`) continue to work unchanged. They internally create a job and wait for completion.
...
2. Adopt async job endpoints (`POST /jobs`) for long-running workflows instead of synchronous endpoints.
3. Use discovery endpoints to dynamically query available strategies, presets, and execution models.
4. Check for `X-Deprecation` headers in API responses and address flagged patterns.
