...
- `Position`, `ClosedTrade`: sealed records for portfolio state. `ClosedTrade` includes always-tracked `MaxAdverseExcursion` and `MaxFavorableExcursion` fields (absolute currency P&L extremes, default 0) plus an optional `TradeAnatomy? Anatomy` field populated when `TraceOptions.EnableEventTrace` is active.
- `TradeAnatomy`: sealed record with `MaxAdverseExcursion` (decimal?), `MaxFavorableExcursion` (decimal?), and `Duration` (TimeSpan) — intra-trade analytics expressed as fractions of entry value, computed by `TradeExcursionTracker`
